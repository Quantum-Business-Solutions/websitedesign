...
| Step | Who | Where | How |
|---|---|---|---|
| 1. Intake and first build | Any teammate | Client Command, Web Pages, "New website directions" | Fill the brief (client, domain, the one line a visitor must believe, ground, accent, cities, service lines, likes, notes). Client Command dispatches `.github/workflows/directions.yml`, which reads their site, writes `brands/<slug>.content.py` by `/directions` (the same rules as `/preview`, nobody watching), renders the three directions and copies them into a site in Client Command; the brand files land on `main`. Twenty to forty minutes; progress shows under the site. Claude Code on this repo still works for the hands-on path. |
| 1b. Intake by hand | Any teammate with Claude Code and access to this repo | Claude Code on `websitedesign` | Say "new client <name>, <domain>, <cities>" and follow INTAKE.md. `scripts/newclient.py` scaffolds the content, search and build files; the fifteen-minute path in RUNBOOK.md does the rest. Locations come from the client's own site and the client, never from a guess. |
...
3. **"New website directions" in Web Pages** (built 9 September 2026): the intake form from INTAKE.md. Submitting it dispatches this repo's `directions.yml` through GitHub's API with the team's GitHub token (Internal Services → GitHub) and the workflow reports back with its OIDC token; the site appears in Web Pages and on the portal's Websites card. Also on the MCP as `build_site_directions`. Setup once: the GitHub token, and `ANTHROPIC_API_KEY` as an Actions secret here.
