...
Gate rerun after the module fixes (Showcase home, copiers, multifunction): the module library had four real failures, all fixed the same hour. 13px floor on mobile text (eight 12.5px labels), the wheel SVG carried role="img" while holding focusable links (nested-interactive; now role="group"), the hero cards used h3 before any h2 (now a styled paragraph), and two seven-item lists left one card alone on a row (wheel list at desktop, hotspot list at tablet; the odd last item now spans). The gate also crashed on SVG anchors, fixed in verify.mjs. Final Showcase home run: 40 PASS, 3 WARN, 1 FAIL, the FAIL being the local LCP artifact from blocked Google Fonts. Remaining WARNs are the header link height and the 30px hotspot dots on phones, which have a 44px list under them. Note for scripts: never `pkill -f` a pattern that appears in the calling command line; it kills the shell running the chain.

Hero motion (2026-09-08, later): the Showcase hero now carries three layers of motion on top of the pointer glow: a 38-second Ken Burns drift on the photo plate, a canvas network of green nodes and lines (56 nodes at desktop, 22 on phones, paused when the hero is off screen), and the Sharp-style 3D device on a slow turntable with a float. Card B's links moved under the copy as a "fast lane" strip so the device owns the stack. The poster under model-viewer must be a transparent PNG on a dark hero; the white JPEG poster read as a pasted square. All motion stops under prefers-reduced-motion.

Depth layer (2026-09-08, evening): a site-wide layer in preview_modules.py adds staggered scroll reveals below the first section, a specular highlight on tilting cards, a perspective floor and pointer spotlight under the proof number (Showcase), floating fleet devices over floor shadows, a glass header on scroll, hero load choreography (Showcase), and a scroll-driven 3D story where the device turns to a new camera orbit for each step (Showcase home and the multifunction page). Two defects caught by the gate and axe before push: inactive story steps at 35 percent opacity failed contrast on 18 nodes (now colour-based, muted ink at full opacity), and revealed elements that a reader scrolled past or jumped over via an anchor stayed invisible (the observer now also shows anything already above the viewport, and a hashchange handler sweeps). A section id in a compose list must be unique on the page: "story" already named the Press timeline, so the device story is "device".

Wheel (2026-09-08, late): Shawn said the Kelly wheel could look better. Redesigned with a radial gradient on the segments, an accent gradient and drop shadow on the active one, segments that lift outward toward the pointer, numerals on the outer edge, a slow dashed orbit ring, tick marks, a gradient hub with a pulse, a big faint numeral in the panel and a progress bar for the autoplay. Found while doing it: the wheel's script had never been emitted (the renderer's return omitted it), so hover swap and autoplay had never worked in any build, and the section and its container shared one id. Both fixed; the browser now reports one active segment and one active list item at load. Hub label derives from the client name rather than the Kelly default, which had leaked into the Nexus preview.

Design system, generated (2026-09-08, late): `scripts/preview_design.py` now writes design-system.html for every client from the same tokens the pages use, the section types the content actually uses, the pitch's "heard" lines and an optional assets/logo-concepts/concepts.json. Kelly's hand-written page was replaced by the generated one with its logo notes carried over as data. Found on the first render: light swatches had white labels on white (label colour now follows luminance), the header logo lacked width and height (flagged by the image check), and "3 directions" read as a digit. The compose check is now strict: a compose id with no section stops the build with the page and direction named.

## Search and AI-answer package (2026-09-08, evening)

- `seo_audit.py` counted `alt=""` as missing. An alt attribute that is present but empty marks a decorative image and is correct; only a missing attribute counts now.
- City-in-title matching was substring based, so "intelligence" matched NC. Word-boundary matching in both the auditor and `normalize_meta`.
- Seven stale blog files from an earlier Kelly build were still in the direction folders and scored as pages. Files not in `content.json` are deleted before a rebuild.
- Titles that fit only without the city were chosen over shorter city-bearing forms. `normalize_meta` now tries city-bearing forms first and shortens the head before dropping the city.
...
## Nexus at the root, and the reveal layer (2026-09-08, night)

- nexus-gold-beta.vercel.app opened on the July canvas set with five old directions; the new site lived under /preview. The preview now sits at the root, the canvases are archived under `archive/canvases-2026-07/`, `/preview/*` redirects, and noindex covers every path. Rule: one site per Vercel project, the hub at the root, nothing older linked or served at the top level.
- Reveal-on-scroll left sections blank in a full-page capture and would do the same on a fast scroll, an anchor jump or a print. The depth layer now reveals ahead of the viewport (35 percent root margin), on every scroll for anything within 1.4 viewports, after 1.2 seconds for anything near the fold, and always in print. Content is never parked hidden.
...
## Charts from data, and the axe pass (2026-09-09, later)

- Shawn asked whether Semrush could supply graphics to break up the text. It cannot; the MCP returns rows. The report now draws its own: a gauge, three donuts, a grade comparison, opportunity bars with position tags, the answer-engine matrix and a scatter of authority against visits. All read from `traffic`, `competitors`, `opportunities`, `aeo` and the two audits, so a new client gets them for free once the seo.json has a `traffic` block (home_share, brand_share, nonbrand_target, with the two notes).
- The first scatter used a log scale from zero; with visits between 160 and 773 every dot sat in the top quarter and two labels printed on top of each other. It is now linear from zero with four round ticks, labels swap to the left on the right half of the plot, and a label that would collide is pushed down with a leader line.
- The full axe run on the report found forty-eight contrast failures, all one cause: text in the raw accent, and ink computed against white while half the sections sit on the tinted ground. Ink is now computed against the tint, the tags against their own tint, and `figure role="img"` was dropped (axe rejects the role on figure; the aria-label stays). The report and the hub return zero violations for Kelly, and the same renderer serves Nexus and Image 2000.
- The open-in-new-tab and full-page request is a ClientCommand change (the report is embedded there); the standalone `seo-report.html` on each Vercel project already is the full-page version.
- Shawn asked for the page paths in the ClientCommand analysis to be clickable. In the standalone report every path in the worst-first table, the money-page crawl table and the competitor-page table is now an anchor to the live page, opening in its own tab (`_link()` in `preview_seo.py`); the every-page cards already linked live and build. ClientCommand's copy is a port of this renderer and has to be re-synced to pick the links up.

## GoodSuite, the first analysis before a build (2026-09-09)

- Shawn asked why the report says pages have no FAQ when the site plainly shows them. The check measures FAQPage schema, which is what a crawler and an answer engine read; GoodSuite's Divi accordions carry none. The auditor now reports that state as "shown, no schema" so the report says what the site does and what it lacks, and the fix reads as markup rather than new copy. Ninety-nine of 317 GoodSuite pages are in that state, including the home page, all ten service pages and the FAQ page itself.
- Shawn asked that the cities never be assumed. The site's service-areas page lists Woodland Hills as headquarters; Yelp and Facebook agree; Shawn gave the real address in Chatsworth. That disagreement is now a High finding rather than a wrong assumption in the report, and the starter config says to take locations from the client's own site and the client.
