...
- The report carries six data-drawn charts, all from numbers already in the seo.json and audit.json, so none needs an image or a Semrush export: a readiness gauge (today against the build), three traffic donuts (home-page share, brand share, the non-brand target) from `traffic`, pages by grade today against the build, opportunity bars with the client's position on each term, the answer-engine matrix (ten cells per query, the client's cell lit) and the authority-against-visits scatter. Semrush returns numbers, not graphics; the renderer draws them. Add a new chart in `preview_seo.py` as a function returning inline SVG or a grid, and it appears on every client at the next build.
- Every report and hub is run through `node scripts/_axefull.mjs <file>` (all axe rules) before it is called done; `_axerule.mjs <file> <rule>` groups the failures of one rule with the measured colours. Text in the brand colour uses `--ink` (darkened until it clears 4.5 to 1 on the tinted section ground, not just white); the tags use `--ink2`, darkened against their own tint. The page has a header and a main landmark. For a look at one region, `node scripts/_clip.mjs <file> <width> <selector> <offset> <height> <out.jpg>` clips it after scrolling it into view; `_clipscroll.mjs` scrolls the whole page first so reveal sections are open.
- Analysis before a build (2026-09-09): `scripts/analyze.py <slug>` renders the same report for a prospect with no directions yet, into `analyses/<slug>/` inside this repo, which the `qbs-analyses` Vercel project serves. The report, the every-page cards and the workbook are identical to a build's minus the build column, so a prospect who signs loses nothing. See INTAKE, "Analysis before a build".
- Every table in the report exports to one Excel workbook, `seo-audit-pages.xlsx`, next to the CSV: Every page (every check, every fix, the new path when there is a build), Findings, Moves, Opportunities, Questions, AI answers, Competitors, Backlinks, Site checks, Redirects. The download buttons sit above the every-page cards.
- The auditor now tells apart a FAQ the visitor can see (an accordion, a list of questions) from FAQPage schema. A page with the first and not the second reads "shown, no schema" in amber, not "no" in red, because the fix is markup, not copy. GoodSuite has 99 such pages.
- Every page path in the report is a link to the live page, opening in its own tab, so a reader can check any row against the site.
- Team self-service (2026-09-09): any teammate rebuilds or re-renders from the GitHub Actions tab ("Client site" workflow) and attaches the link to the portal in Client Command. TEAM.md says who does what today and the four things Client Command adds so a site lives inside it. Kelly's hub and report are attached to its portal.
- The entity graph is scored (2026-09-11): a second score out of 100 beside readiness, on every page, in the hero, in its own report section and in the workbook. seo-baseline.md version 3 has the checks and the three rules that keep the number honest. It scores a prospect's existing site on the thing the engagement sells, and it scores our own build the same way, which is how the missing WebPage, dateModified and Person nodes in our builder were found.
- Naming people: `schema.people` in the content file emits Person nodes with worksFor and becomes the author of every post. Leave it empty rather than invent anyone; the entity check fails honestly and the client is asked for the names.
