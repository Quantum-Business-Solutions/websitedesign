...
> **Every tier clones. There is no "use the theme as-is" tier.** An earlier version of this file said
> Launch needed no clone, which was incoherent: the launch gate requires `appearance.mode` set
> explicitly and the `Organization` schema naming the client, and on an un-cloned theme the only way
> to do either is to edit one of the nine — which `process/reskin.md` forbids, and which would change
> the product for every other client on that theme. Cloning is now a scripted ten-minute operation
> (`scripts/reskin.py`), so the reason to skip it is gone.
